...
function(url) {
	...
	_loginHandler: function(response) {
		...
		if (response.result && response.pageId) {
			window.name = response.pageId;
		} else {
			window.name = null;
		}
		tjs.lang.destroyObject(response);
	...
	_doLogin: function(userid,password) {
		...
		var hmac = CryptoJS.algo.HMAC.create(CryptoJS.algo.SHA256, psw);
		hmac.update(userid);
...
function(url) {
	this.url = url;
	this._logoutHandler_ = this._logoutHandler.bind(this);
	this._firstLoginHandler_ = this._firstLoginHandler.bind(this);
},{
	destroy: function() {
		tjs.lang.destroyObject(this);
	},
	logout: function() {
		window.name = null;
		tjs.net.httpPOST4Json(this.url,'cmd=logout',this._logoutHandler_);
	},
	...
	checkPageId: function(f) {
		if (tjs.net.getPageId()) {
			tjs.net.httpPOST4Json(this.url,'cmd=checkPageId',function(response) {
				if (response.result) {
					if (tjs.lang.isFunction(f)) {f();}
				} else {
					window.name = null;
					window.location.href = '/error/PageError.html';
				}
				tjs.lang.destroyObject(response);
			});
		} else {
	...
	_firstLoginHandler: function(response) {
		if (response.message) {
			alert(response.message);
		}
	...
	firstLogin: function(){
		tjs.net.httpPOST4Json(this.url,'cmd=first_login',this._firstLoginHandler_);
	}
});
