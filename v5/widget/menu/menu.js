...
//tjs_debug_end
		...
		this.oElement.className = 'tjs_menu tjs_menu_'+this._clsId+' tjs_menu_'+this._hv+' tjs_menu_'+this._clsId+'_'+this._hv;
		if (this.oNodes) {
			...
			this._oldChain = null;
			var q = this._createMenuNodes(this.oElement,this.oNodes);
			if (q.length > 0) {
				var i = q.length,n;
				while (i--) {
					n = q[i];
	...
	_createMenuNodes:function(oMenu,oNodes) {
		var q = [], k = 0, oNode, data;
	...
	_createPopupMenu:function(oNode){
		...
		var q = this._createMenuNodes(oMenu,oNode.children);
		...
		oNode.oPopup = {oElement:oElement,oMenu:oMenu};
